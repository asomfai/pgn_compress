...
#DONE: decoding
#todo: make e.p. and castling a different class
...
############################# define move #############################
class Move:
	def __init__(self, piece, to_x,to_y,capture=False,castling=False):
		self.piece = piece
		self.to_x = to_x
		self.to_y = to_y
		...
		if not self.castling and self.to_y not in range(0,8):raise Exception("Bad move to",self.to_x,self.to_y,capture)
		if self.capture not in [True,False] :raise Exception("Bad move to",to_x,to_y,capture)
		
	def toString(self):
		string = self.piece.toString()
		if self.castling:
			if self.to_x==2: string += " O-O-O to"+str(self.to_x)+str(self.to_y)
			else: string += " O-O"+str(self.to_x)+str(self.to_y)
		else:
	...
	def Match(self, other):
		...
		if self.to_x != other.to_x or not self.castling and self.to_y != other.to_y : return False
		return self.castling or self.piece.Type == other.piece.Type
		...
		for rook_file,castling_direction in [(0,-1),(7,+1)]:
			...
			if table[rook_file][y]!=None and table[rook_file][y].Moved==False and table[rook_file][y].isRook():
				...
				if castling_possible:
					steps.append(Move(piece,x+2*castling_direction,y,False,True))
	...
	for move in moves:
		if move.to_x==x_to and move.to_y==y_to:
			...
			if move.castling:
				...
				if table[x_to][y_to].Color!=table[x_from][y_from].Color:
...
def SetupTable():
	...
	table[0][0]=table[7][0]=Piece('w','R')
	table[1][0]=table[6][0]=Piece('w','N')
	table[2][0]=table[5][0]=Piece('w','B')
	table[3][0]=Piece('w','Q')
	table[4][0]=Piece('w','K')
...
#	print type,trg_file,trg_rank,dis_file,dis_rank,capture
	...
	move = Move(Piece(color,type), trg_file, trg_rank, capture, queen_castling or king_castling)
...
def ProcessPGN(file_name):
	...
	while True:
		...
		for move in moves:
			...
			if verbose:print "players move: ",move.toString(),"possible moves",len(possible_moves)
